...
import { 
  Rocket, GithubIcon, CheckCircle, Loader2, Link as LinkIcon, Download 
} from 'lucide-react';
...
const ActionHeader: React.FC = () => {
  ...
  const [showToast, setShowToast] = useState<'sync' | 'deploy' | null>(null);
  ...
  const handleDeploy = () => {
    ...
    setTimeout(() => {
      setDeploying(false);
      setLiveUrl(`https://${portfolioData.name.toLowerCase().replace(/\s/g, '-') || 'user'}.portfoli.ai`);
      setShowToast('deploy');
        ...
        <div className="fixed bottom-6 right-6 bg-slate-800 border border-slate-700 p-4 rounded-xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-right-10 z-[120]">
          ...
          <div>
            <p className="font-bold text-slate-50">
              {showToast === 'sync' ? 'GitHub Repos Synced' : 'Site Deployed Successfully'}
            </p>
            <p className="text-xs text-slate-400">
              {showToast === 'sync' ? 'Your latest projects have been pulled.' : 'Your portfolio is live globally.'}
            </p>
