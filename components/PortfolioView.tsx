...
import { Github, Linkedin, Mail, ExternalLink, Code, Palette, Zap, Menu, X, ArrowUp, Instagram, Twitter } from 'lucide-react';
      ...
      <div id="main-content" className={`max-w-6xl mx-auto px-6 pt-32 relative z-10`}>
        ...
        <header className={`py-20 lg:py-32 px-4 ${theme.header}`}>
          ...
          <h1 className={`text-6xl md:text-9xl font-bold mb-8 tracking-tighter leading-none text-white`}>
            {data.name || (data.purpose === 'Business' ? 'Business Entity' : 'Anonymous')}
          </h1>
          <p className={`text-xl md:text-2xl text-slate-400 mb-12 max-w-2xl leading-relaxed font-light`}>
            {data.title || (data.purpose === 'Business' ? 'Industry Innovator' : 'Professional Specialist')}
          </p>
          <div className="flex flex-wrap gap-4 justify-start">
            {[
              { Icon: Github, value: data.github, label: 'GitHub' },
              { Icon: Linkedin, value: data.linkedin, label: 'LinkedIn' },
              { Icon: Mail, value: data.email, label: 'Email' },
              { Icon: Instagram, value: data.instagram, label: 'Instagram' },
              { Icon: Twitter, value: data.twitter, label: 'Twitter' }
            ].filter(s => s.value).map(({ Icon, label }, i) => (
              <a
                key={i}
                href="#"
          ...
          <aside className="lg:col-span-4 space-y-24">
            <section id="experience" className="scroll-mt-32">
              <h2 className={`text-[10px] font-mono uppercase tracking-widest mb-12 text-slate-500 border-b border-white/10 pb-4`}>
                03 // {data.purpose === 'Business' ? 'OUR_STORY' : 'EXPERIENCE_LOG'}
              </h2>
